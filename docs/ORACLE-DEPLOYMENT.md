...
Déploiement prévu : VM Ubuntu ARM Ampere A1, Docker Compose, Caddy HTTPS, API Node.js et MySQL. Aucun serveur Oracle n’a encore été connecté ni déployé depuis cette session.
...
Avant toute réinitialisation d’une base existante : identifier l’hôte et le nom de base, sauvegarder, puis arrêter le backend. La base locale n’est pas implicitement la cible du déploiement Oracle. Supprimer des utilisateurs de la base ne supprime pas leurs identités dans Firebase Authentication.
